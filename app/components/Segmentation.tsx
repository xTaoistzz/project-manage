...
import Polygon from "@/app/components/Polyg"
...
const Detection = ({ idproject }) => {
  ...
  const [idsegmentation, setSegmentation] = useState("")
  ...
  const path_commpare = (url) => {
    ...
    newData.forEach((com) => {
      if (com.image_path === image_path) {
        setSegmentation(com.idsegmentation)
        console.log(com.idsegmentation)
      }
    }
    );
  }
        ...
        <div className="grid grid-cols-6 gap-6 w-full p-3 h-full">
          <div className="col-span-6 border-blue-950 border-8 flex justify-center items-center">
            <div className="text-center">
              {active && (
                <Polygon imageUrl={active.replace("thumbs", "images")} idproject={idproject} idsegmentation={idsegmentation} />
              )}
            </div>
          </div>
        </div>
